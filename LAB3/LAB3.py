...
# 2. Function to return a sublist of movies with IMDB score above 5.5
def get_highly_rated_movies(movies):
    return [movie for movie in movies if movie["imdb"] > 5.5]

# 3. Function to get movies of a specific category
def get_movies_by_category(movies, category):
    return [movie for movie in movies if movie["category"] == category]

# 4. Function to compute average IMDB score of a list of movies
def average_imdb_score(movies):
    return sum(movie["imdb"] for movie in movies) / len(movies) if movies else 0

# 5. Function to compute average IMDB score for a given category
